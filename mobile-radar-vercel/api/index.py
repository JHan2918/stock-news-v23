    ...
    try:
        ...
        with con.cursor() as cur:
            cur.execute(
                ...
                FROM public.device_watchlists
                WHERE device_id=%s
                ORDER BY sort_order
                """,
                (device_id,),
            )
            return [{"name": r[0] or "", "code": r[1] or ""} for r in cur.fetchall()]
    finally:
        con.close()


def save_pg_watchlist(device_id, stocks):
    con = pg_connect()
    if not con:
        return False
    try:
        ensure_pg_schema(con)
        with con.cursor() as cur:
            cur.execute("DELETE FROM public.device_watchlists WHERE device_id=%s", (device_id,))
            for idx, stock in enumerate((stocks or [])[:3], 1):
                cur.execute(
                    """
                    INSERT INTO public.device_watchlists(device_id, stock_name, stock_code, sort_order, updated_at)
                    VALUES (%s, %s, %s, %s, NOW())
                    """,
                    (device_id, stock.get("name") or "", stock.get("code") or "", idx),
                )
        return True
    finally:
        con.close()
...
def pg_visit_counts(visitor_id, today):
    con = pg_connect()
    if not con:
        return None
    try:
        ...
        with con.cursor() as cur:
            cur.execute(
                """
                INSERT INTO public.visitor_logs(device_id, visit_date, first_seen_at, last_seen_at)
                VALUES (%s, %s, NOW(), NOW())
                ON CONFLICT (device_id, visit_date)
                DO UPDATE SET last_seen_at=EXCLUDED.last_seen_at
                """,
                (visitor_id, today),
            )
            cur.execute("SELECT COUNT(DISTINCT device_id) FROM public.visitor_logs WHERE visit_date=%s", (today,))
            today_count = int(cur.fetchone()[0] or 0)
            cur.execute("SELECT COUNT(DISTINCT device_id) FROM public.visitor_logs")
...
def save_supabase_watchlist(device_id, stocks):
    try:
        if save_pg_watchlist(device_id, stocks):
            return True
    except Exception:
        pass
    try:
...
def visit_payload(handler):
    ...
    try:
        counts = pg_visit_counts(visitor_id, today)
        if counts:
            payload = {
                "ok": True,
                "today": today,
                "todayVisitors": counts[0],
                "totalVisitors": counts[1],
                "storage": "postgres",
            }
            headers = None
            if new_cookie:
...
def report_zip_path():
    ...
    remote = os.environ.get(
        "REPORT_DB_URL",
        "https://raw.githubusercontent.com/JHan2918/stock-news-v23/main/data/report_reports.db.zip",
    )
    if remote:
        ...
        try:
            if os.path.exists(cache) and os.path.getsize(cache) > 0:
                ...
                if age < ttl:
                    return cache
            req = Request(remote, headers={"User-Agent": "market-radar-mobile-vercel/1.0"})
            with urlopen(req, timeout=45) as res, open(cache + ".tmp", "wb") as out:
                while True:
            ...
            with open(meta, "w", encoding="utf-8") as f:
                f.write(f"{datetime.now(KST).isoformat()} {remote}")
            return cache
        except Exception:
            if os.path.exists(cache) and os.path.getsize(cache) > 0:
                return cache
...
<div class="app">
  <div class="top">
    <div class="top-actions"><button onclick="showDetail('settings')">설정</button></div>
    <h1>시장 레이더 Mobile</h1>
...
function defaultWatchlist(){return [{name:"삼성전자",code:"005930"},{name:"SK하이닉스",code:"000660"},{name:"현대차",code:"005380"}]}
function loadLocalWatchlist(){try{const rows=JSON.parse(localStorage.getItem("mr_watchlist_v1")||"[]");return Array.isArray(rows)&&rows.length?rows.slice(0,3):defaultWatchlist()}catch(e){return defaultWatchlist()}}
function saveLocalWatchlist(rows){localStorage.setItem("mr_watchlist_v1",JSON.stringify((rows||[]).slice(0,3)))}
...
async function loadVisitStats(){const el=document.getElementById("visitorStats");if(!el)return;try{const r=await fetch("/api/visit?ts="+Date.now());const d=await r.json();if(!d.ok)throw new Error(d.error||"방문자 로드 실패");if(d.storage==="postgres"||d.storage==="supabase"){el.textContent=`오늘 방문 ${num(d.todayVisitors)}명 · 누적 ${num(d.totalVisitors)}명`}else{el.textContent="방문자 DB 연결 확인 중"}}catch(e){el.textContent="방문자 DB 연결 확인 중"}}
...
function reportSlashItems(text){return String(text||"").split(/\s*\/\s*|\n+/).map(x=>cleanReportText(x)).filter(x=>x&&x.length>=3)}
function reportCoreItems(items){const out=[];const keys=[];(items||[]).forEach(t=>{reportSlashItems(t).forEach(part=>{part=part.replace(/^투자의견\s*[^\s]+\s*/,"").trim();const k=part.replace(/[\s.,:;\/()\[\]{}-]/g,"").toLowerCase();if(!k||k.length<5)return;if(keys.some(x=>k===x||k.includes(x)||x.includes(k)))return;keys.push(k);out.push(part)})});return out}
function reportDetailTextBlock(summaryParts,report){const reasonTexts=(report.reasons||[]).map(x=>x&&x.reason_text||"");const coreItems=reportCoreItems([summaryParts.core,report.target_price_reason,report.risk_summary,...reasonTexts]).slice(0,8);const headline=summaryParts.head||report.title||"";const opinion=report.investment_opinion||summaryParts.opinion.replace(/^투자의견\s*/,"");const coreHtml=coreItems.length?`<div style='margin-top:10px'><b>핵심</b><div class='report-core-list'>${coreItems.slice(0,5).map(x=>`<div class='report-core-item'>${esc(x)}</div>`).join("")}</div></div>`:"";return `<div class='section-note'>${headline?`<div class='report-detail-title'>${esc(headline)}</div>`:""}${opinion?`<div class='report-detail-line' style='margin-top:8px'><b>투자의견</b> ${esc(opinion)}</div>`:""}${summaryParts.head?`<div class='report-detail-line'><b>요약</b> ${esc(summaryParts.head)}</div>`:""}${coreHtml}</div>`}
function renderReportDetail(d,report={}){const close=d.closeSeries||[];const targets=d.targetSeries||[];const flows=d.flowSeries||[];const kws=(report.keywords||[]).slice(0,8).map(k=>`<span class='chip'>${esc(k.keyword)}</span>`).join("");const summaryParts=splitReportSummary(report.summary||"");const upside=report.upside_potential?`${Number(report.upside_potential).toFixed(1)}%`:"-";const opinion=report.investment_opinion?`<div class='chart-pill'><span>투자의견</span><b>${esc(report.investment_opinion)}</b></div>`:"";const target=report.target_price?`<div class='chart-pill'><span>목표가</span><b>${num(report.target_price)}원</b></div>`:"";const previous=report.previous_target_price?`<div class='chart-pill'><span>이전 목표가</span><b>${num(report.previous_target_price)}원</b></div>`:`<div class='chart-pill'><span>이전 목표가</span><b>-</b></div>`;const current=report.current_price_at_report_date?`<div class='chart-pill'><span>현재가</span><b>${num(report.current_price_at_report_date)}원</b></div>`:"";const upsidePill=`<div class='chart-pill ${report.upside_potential?Number(report.upside_potential)>=0?"good":"bad":""}'><span>상승여력</span><b>${esc(upside)}</b></div>`;return `<div class='section-note'><b>${esc(report.stock_name||d.stockName||d.stockCode)}</b> <span class='meta'>${esc(report.stock_code||d.stockCode||"")}</span><br>${esc(report.securities_firm||"")} / ${esc(report.report_date||d.reportDate||"")}</div><div class='chart-summary'>${target}${previous}${current}${upsidePill}</div>${reportDetailTextBlock(summaryParts,report)}${kws?`<div class='section-note'><b>키워드</b><br>${kws}</div>`:""}<div class='section-note'>종가 ${close.length}일 / 목표가 ${targets.length}건 / 수급 ${flows.length}일</div><div class='meta'>종가 vs 목표가 추이</div>${priceTargetSvg(close,targets)}<div class='meta'>외국인/기관 순매수 추이</div>${flowSvg(flows)}`}
...
function watchInputHtml(i,w){const row=w[i-1]||{};return `<label class='full'>관심종목 ${i}<span class='watch-suggest-wrap'><input id='watchStock${i}' name='stock${i}' value='${esc(row.name||row.code||"")}' placeholder='삼, 삼성, 005930' autocomplete='off'><input id='watchCode${i}' name='stock${i}_code' type='hidden' value='${esc(row.code||"")}'><div id='watchSuggest${i}' class='watch-suggestions hidden'></div></span></label>`}
function renderMemberPage(){const el=document.getElementById("detailList");const d=MEMBER_DATA||guestMemberPayload();const m=d.member||{};const w=(d.watchlist&&d.watchlist.length?d.watchlist:loadLocalWatchlist()).slice(0,3);el.innerHTML=`<div class='section-note'><b>관심종목 설정</b><br>종목명 일부만 입력하면 자동추천이 나옵니다. 선택한 종목은 이 브라우저와 Supabase DB에 저장됩니다.</div><form class='member-form' onsubmit='saveMember(event)'>${watchInputHtml(1,w)}${watchInputHtml(2,w)}${watchInputHtml(3,w)}<div id='memberSaveMsg' class='save-msg'></div><button>관심종목 저장</button></form>`;bindWatchSuggest()}
function bindWatchSuggest(){[1,2,3].forEach(i=>{const input=document.getElementById(`watchStock${i}`);const code=document.getElementById(`watchCode${i}`);const box=document.getElementById(`watchSuggest${i}`);if(!input||!box)return;let timer=null;input.addEventListener("input",()=>{code.value="";clearTimeout(timer);const q=input.value.trim();if(!q){box.classList.add("hidden");box.innerHTML="";return}timer=setTimeout(async()=>{try{const r=await fetch(`/api/stocks?q=${encodeURIComponent(q)}&limit=8&ts=${Date.now()}`);const d=await r.json();const items=d.items||[];if(!items.length){box.classList.add("hidden");return}box.innerHTML=items.map(it=>`<div class='watch-suggestion' onclick='pickWatchStock(${i},"${esc(it.name)}","${esc(it.code)}")'>${esc(it.name)}</div>`).join("");box.classList.remove("hidden")}catch(e){box.classList.add("hidden")}},160)})})}
function pickWatchStock(i,name,code){document.getElementById(`watchStock${i}`).value=name;document.getElementById(`watchCode${i}`).value=code;document.getElementById(`watchSuggest${i}`).classList.add("hidden")}
        ...
        try:
            ...
            if parsed.path == "/api/member/update":
                ...
                if not member:
                    ...
                    selected_stocks = []
                    for idx in (1, 2, 3):
                        raw = data.get(f"stock{idx}_code") or data.get(f"stock{idx}")
                        stock = resolve_watch_stock(raw)
                        if stock:
                            selected_stocks.append(stock)
                    warm_stock_analysis(selected_stocks)
                ...
                name = str(data.get("name") or "").strip()
                if not name:
                    name = str(member.get("name") or "회원")
                con = member_connect()
                selected_stocks = []
                try:
                    con.execute(
                    ...
                    for idx, raw in enumerate([data.get("stock1_code") or data.get("stock1"), data.get("stock2_code") or data.get("stock2"), data.get("stock3_code") or data.get("stock3")], 1):
                        stock = resolve_watch_stock(raw)
                        if stock:
                            selected_stocks.append(stock)
                            con.execute(
    ...
    def do_GET(self):
        try:
            ...
            if parsed.path == "/login":
                return self.send(200, HTML, headers={"Set-Cookie": ["mr_session=; Path=/; Max-Age=0; HttpOnly; SameSite=Lax", "mr_guest=1; Path=/; SameSite=Lax; Max-Age=31536000"]})
