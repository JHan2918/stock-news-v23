...
function bindWatchSuggest(){[1,2,3].forEach(i=>{const input=document.getElementById(`watchStock${i}`);const code=document.getElementById(`watchCode${i}`);const box=document.getElementById(`watchSuggest${i}`);if(!input||!box)return;let timer=null;input.addEventListener("input",()=>{code.value="";clearTimeout(timer);const q=input.value.trim();if(!q){box.classList.add("hidden");box.innerHTML="";return}timer=setTimeout(async()=>{try{const r=await fetch(`/api/stocks?q=${encodeURIComponent(q)}&limit=10&ts=${Date.now()}`);const d=await r.json();const items=d.items||[];if(!items.length){box.classList.add("hidden");return}box.innerHTML=items.map(it=>`<div class='suggestion' onclick='pickWatchStock(${i},"${esc(it.name)}","${esc(it.code)}")'><b>${esc(it.name)}</b><span>${esc(it.code)}</span></div>`).join("");box.classList.remove("hidden")}catch(e){box.classList.add("hidden")}},160)})})}
