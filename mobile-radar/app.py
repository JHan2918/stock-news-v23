...
function renderMobileThemes(d){const el=document.getElementById("detailList");const themes=d.themes||[];let html=`<div class='section-note'>${esc(d.start||"-")} ~ ${esc(d.end||"-")} / ${esc(d.provider||"")} / 테마 ${themes.length}개</div>`;html+=themes.map((t,i)=>{const stocks=(t.stocks||[]).slice(0,5).map(s=>`<div class='mini-bar'><span>${esc(s.name)}</span><div class='bar-track'><span class='bar-fill' style='width:${Math.max(4,Math.min(100,Math.abs(Number(s.changePct||0))*8))}%'></span></div><b class='${Number(s.changePct)>=0?"pos":"neg"}'>${pct(s.changePct)}</b></div>`).join("");return `<div class='metric'><b>${i+1}. ${esc(t.name)}</b><span>점수 ${Number(t.score||0).toFixed(1)} / 상승률 ${pct(t.changePct)} / 외국인+기관 ${money(t.netBuyTotal)}</span><div style='margin-top:6px'>${(t.keywords||[]).map(k=>`<span class='chip'>${esc(k)}</span>`).join("")}</div><div class='mini-bars'>${stocks}</div></div>`}).join("");el.innerHTML=html}
