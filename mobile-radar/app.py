...
function reportDetailTextBlock(summaryParts,report){const reasonTexts=(report.reasons||[]).map(x=>x&&x.reason_text||"");const coreItems=reportCoreItems([summaryParts.core,report.target_price_reason,report.risk_summary,...reasonTexts]).slice(0,8);const headline=summaryParts.head||report.title||"";const opinion=report.investment_opinion||summaryParts.opinion.replace(/^투자의견\s*/,"");const coreHtml=coreItems.length?`<div style='margin-top:10px'><b>핵심</b><div class='report-core-list'>${coreItems.slice(0,5).map(x=>`<div class='report-core-item'>${esc(x)}</div>`).join("")}</div></div>`:"";const chipHtml=coreItems.length?`<div class='report-core-chips'>${coreItems.slice(0,8).map(x=>`<span class='chip'>${esc(x.length>18?x.slice(0,18)+"...":x)}</span>`).join("")}</div>`:"";return `<div class='section-note'>${headline?`<div class='report-detail-title'>${esc(headline)}</div>`:""}${opinion?`<div class='report-detail-line' style='margin-top:8px'><b>투자의견</b> ${esc(opinion)}</div>`:""}${summaryParts.head?`<div class='report-detail-line'><b>요약</b> ${esc(summaryParts.head)}</div>`:""}${coreHtml}${chipHtml}</div>`}
