...
.ticker{height:47px;overflow:hidden}.ticker-track{display:grid;gap:5px;animation:roll 10s linear infinite}.ticker-line{display:grid;grid-template-columns:18px minmax(0,1fr) auto;gap:5px;align-items:center;color:#d8e4ee;font-size:12px;min-height:21px}.ticker-line span:nth-child(2){white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ticker-rank{color:var(--accent);font-weight:900}.ticker-val{color:var(--good);font-weight:800;font-size:11px;white-space:nowrap}@keyframes roll{0%,20%{transform:translateY(0)}28%,48%{transform:translateY(-26px)}56%,76%{transform:translateY(-52px)}84%,96%{transform:translateY(-78px)}100%{transform:translateY(0)}}.hint{font-size:11px;color:var(--muted);margin-top:7px}.report-home{height:82px;display:grid;grid-template-columns:70px 1fr;gap:10px;align-items:center}.report-art{height:74px;border-radius:15px;background:#0d131a url('/static/report-card-d.png') center/contain no-repeat;border:1px solid #344151;box-shadow:0 12px 24px rgba(125,177,255,.18)}.report-home b{display:block;color:#d7e7ff;font-size:14px}.report-home span{display:block;color:#9fb0bf;font-size:11px;line-height:1.45;margin-top:3px}
...
function priceTargetSvg(close,targets){if(!close.length&&!targets.length)return "<div class='empty'>종가·목표가 데이터가 없습니다.</div>";const w=320,h=170,p=22;const dates=[...new Set([...close.map(x=>x.date),...targets.map(x=>x.date)].filter(Boolean))].sort();const vals=[...close.map(x=>Number(x.close)),...targets.map(x=>Number(x.targetPrice))].filter(Boolean);const min=Math.min(...vals),max=Math.max(...vals),span=max-min||1;const xOfDate=date=>{const idx=Math.max(0,dates.indexOf(date));return dates.length>1?p+idx*(w-p*2)/(dates.length-1):w/2};const yOf=v=>p+(max-Number(v))/span*(h-p*2);const closeByDate=new Map(close.map(x=>[x.date,x.close]));const pts=dates.filter(d=>closeByDate.has(d)).map(d=>`${xOfDate(d)},${yOf(closeByDate.get(d))}`).join(" ");const dots=targets.map(t=>`<circle cx='${xOfDate(t.date)}' cy='${yOf(t.targetPrice)}' r='3.8' fill='#ff8585'></circle>`).join("");return `<svg class='detail-chart' viewBox='0 0 ${w} ${h}'><line x1='${p}' y1='${h-p}' x2='${w-p}' y2='${h-p}' stroke='#263544'></line><polyline points='${pts}' fill='none' stroke='#7db1ff' stroke-width='2.5'></polyline>${dots}</svg>`}
function flowSvg(flows){if(!flows.length)return "<div class='empty'>외국인/기관 수급 데이터가 없습니다.</div>";const w=320,h=170,p=22;let f=0,i=0;const acc=flows.map(r=>{f+=Number(r.foreignNetAmount||0);i+=Number(r.institutionNetAmount||0);return {date:r.date,f,i}});const vals=acc.flatMap(x=>[x.f,x.i]);const max=Math.max(...vals.map(v=>Math.abs(v)),1);const y=v=>h/2-(v/max)*(h/2-p);const xOf=idx=>acc.length>1?p+idx*(w-p*2)/(acc.length-1):w/2;const fp=acc.map((r,idx)=>`${xOf(idx)},${y(r.f)}`).join(" ");const ip=acc.map((r,idx)=>`${xOf(idx)},${y(r.i)}`).join(" ");return `<svg class='detail-chart' viewBox='0 0 ${w} ${h}'><line x1='${p}' y1='${h/2}' x2='${w-p}' y2='${h/2}' stroke='#344151'></line><polyline points='${fp}' fill='none' stroke='#7db1ff' stroke-width='2.5'></polyline><polyline points='${ip}' fill='none' stroke='#ff8585' stroke-width='2.5'></polyline></svg><div class='meta'>파랑: 외국인 누적 / 빨강: 기관 누적</div>`}
