...
.ticker{height:47px;overflow:hidden}.ticker-track{display:grid;gap:5px;animation:roll 10s linear infinite}.ticker-line{display:grid;grid-template-columns:18px minmax(0,1fr) auto;gap:5px;align-items:center;color:#d8e4ee;font-size:12px;min-height:21px}.ticker-line span:nth-child(2){white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ticker-rank{color:var(--accent);font-weight:900}.ticker-val{color:var(--good);font-weight:800;font-size:11px;white-space:nowrap}@keyframes roll{0%,20%{transform:translateY(0)}28%,48%{transform:translateY(-26px)}56%,76%{transform:translateY(-52px)}84%,96%{transform:translateY(-78px)}100%{transform:translateY(0)}}.hint{font-size:11px;color:var(--muted);margin-top:7px}.report-home,.macro-home{height:82px;display:grid;grid-template-columns:70px 1fr;gap:10px;align-items:center}.report-art,.macro-art{height:74px;border-radius:15px;background-color:#0d131a;background-position:center;background-size:contain;background-repeat:no-repeat;border:1px solid #344151;box-shadow:0 12px 24px rgba(125,177,255,.18)}.report-art{background-image:url('/static/report-card-d.png')}.macro-art{background-image:url('/static/macro-card.png')}.report-home b,.macro-home b{display:block;color:#d7e7ff;font-size:14px}.report-home span,.macro-home span{display:block;color:#9fb0bf;font-size:11px;line-height:1.45;margin-top:3px}
...
.industry-controls{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px}.industry-controls label{display:grid;gap:4px;color:#9fb0bf;font-size:11px}.industry-controls input,.industry-controls select{width:100%;height:38px;border-radius:10px;border:1px solid #344151;background:#0d131a;color:#f2f7ff;padding:0 10px;font-size:13px}.industry-controls .full{grid-column:span 2}.industry-controls button{grid-column:span 2;height:40px;border:0;border-radius:11px;background:#2f81f7;color:#fff;font-weight:900}.industry-chart{width:100%;height:210px;display:block;background:#0b1118;border:1px solid #263544;border-radius:10px;margin:8px 0}.industry-stat-table{width:100%;border-collapse:collapse;font-size:12px}.industry-stat-table th,.industry-stat-table td{border-bottom:1px solid #263544;padding:7px 4px;text-align:right}.industry-stat-table th:first-child,.industry-stat-table td:first-child{text-align:left}.industry-stat-table th{color:#9fb0bf;font-weight:500}
...
function renderReportDetail(d,report={}){const close=d.closeSeries||[];const targets=d.targetSeries||[];const flows=d.flowSeries||[];const reasons=(report.reasons||[]).slice(0,5).map(x=>`<li>${esc(x.reason_keyword||"")} ${esc(x.reason_text||"")}</li>`).join("");const kws=(report.keywords||[]).slice(0,8).map(k=>`<span class='chip'>${esc(k.keyword)}</span>`).join("");const summary=report.summary||"";const targetReason=report.target_price_reason||"";const risk=report.risk_summary||"";return `<div class='section-note'><b>${esc(report.stock_name||d.stockName||d.stockCode)}</b><br>${esc(report.title||"")}<br>${esc(report.securities_firm||"")} / ${esc(report.report_date||d.reportDate||"")} / ${esc(report.investment_opinion||"")}</div>${kws?`<div class='meta'>${kws}</div>`:""}${summary?`<div class='section-note'><b>요약</b><br>${esc(summary)}</div>`:""}${targetReason?`<div class='section-note'><b>목표가 이유</b><br>${esc(targetReason)}</div>`:""}${risk?`<div class='section-note'><b>리스크</b><br>${esc(risk)}</div>`:""}${reasons?`<div class='section-note'><b>핵심 근거</b><ul>${reasons}</ul></div>`:""}<div class='section-note'>종가 ${close.length}일 / 목표가 ${targets.length}건 / 수급 ${flows.length}일</div><div class='meta'>종가 vs 목표가 추이</div>${priceTargetSvg(close,targets)}<div class='meta'>외국인/기관 순매수 추이</div>${flowSvg(flows)}`}
...
function renderMobileThemes(d){const el=document.getElementById("detailList");const themes=d.themes||[];let html=`<div class='section-note'>${esc(d.start||"-")} ~ ${esc(d.end||"-")} / ${esc(d.provider||"")} ?? ?? ${themes.length}?</div>`;html+=themes.map((t,i)=>{const stocks=(t.stocks||[]).slice(0,5).map(s=>`<div class='mini-bar'><span>${esc(s.name)}</span><div class='bar-track'><span class='bar-fill' style='width:${Math.max(4,Math.min(100,Math.abs(Number(s.changePct||0))*8))}%'></span></div><b class='${Number(s.changePct)>=0?"pos":"neg"}'>${pct(s.changePct)}</b></div>`).join("");return `<div class='metric'><b>${i+1}. ${esc(t.name)}</b><span>?? ${Number(t.score||0).toFixed(1)} / ?? ${pct(t.changePct)} / ??+?? ${money(t.netBuyTotal)}</span><div style='margin-top:6px'>${(t.keywords||[]).map(k=>`<span class='chip'>${esc(k)}</span>`).join("")}</div><div class='mini-bars'>${stocks}</div></div>`}).join("");el.innerHTML=html}
function renderStaticRows(rows,type){const el=document.getElementById("detailList");if(!rows.length){el.innerHTML="<div class='empty'>??? ???? ????.</div>";return}el.innerHTML=rows.map((r,i)=>`<div class='row'><div class='rank'>${i+1}</div><div><div class='name'>${esc(r.name)}</div><div class='meta'>${esc(r.value)} / ?? ??? ?? ??</div></div><div class='score'>???</div></div>`).join("")}
function svgLine(series,pct){if(!series||series.length<2)return "<div class='empty'>??? ??? ??</div>";const w=320,h=108,p=12;const vals=series.map(x=>Number(x.value));const min=Math.min(...vals),max=Math.max(...vals);const span=max-min||1;const pts=series.map((d,i)=>{const x=p+i*(w-p*2)/(series.length-1);const y=p+(max-Number(d.value))/span*(h-p*2);return [x,y]}).map(p=>p.join(",")).join(" ");const area=`${p},${h-p} ${pts} ${w-p},${h-p}`;const color=Number(pct)>=0?"#8aff8a":"#ff8585";const fill=Number(pct)>=0?"#163222":"#3a1d25";return `<svg class='mini-svg' viewBox='0 0 ${w} ${h}' preserveAspectRatio='none'><line class='axis' x1='${p}' y1='${h-p}' x2='${w-p}' y2='${h-p}'></line><polygon class='area' style='fill:${fill}' points='${area}'></polygon><polyline class='line' style='stroke:${color}' points='${pts}'></polyline></svg>`}
function renderMacroCharts(){const el=document.getElementById("detailList");const rows=DATA.macroCharts||[];if(!rows.length){el.innerHTML="<div class='empty'>??? ??? ???? ????.</div>";return}const grouped={};rows.forEach(r=>{const k=r.category||"???";(grouped[k]=grouped[k]||[]).push(r)});el.innerHTML=Object.keys(grouped).map(cat=>`<div class='section-note'><b>${esc(cat)}</b></div><div class='macro-chart-grid'>${grouped[cat].map(r=>{const pct=Number(r.pct);const cls=pct>=0?"macro-pos":"macro-neg";const sign=Number.isFinite(pct)?(pct>0?"+":"")+pct.toFixed(2)+"%":"";return `<div class='macro-chart'><div class='macro-chart-head'><div class='macro-chart-name'>${esc(r.name)}</div><div class='macro-chart-value'>${r.latest==null?"-":Number(r.latest).toLocaleString()}${esc(r.unit||"")} <span class='${cls}'>${sign}</span></div></div>${svgLine(r.series,pct)}</div>`}).join("")}</div>`).join("")}
function openModal(type,i){const r=(type==="stock"?DATA.stockHot:DATA.macroHot)[i];if(!r)return;document.getElementById("modalTitle").textContent=type==="stock"?`${r.stockName} ??`:`${r.keyword} ??`;document.getElementById("modalMeta").textContent=`?? ${r.newsCount}? / ?? ${r.score}`;document.getElementById("modalBody").innerHTML=(r.articles||[]).map(a=>`<div class='news'><a href='${esc(a.link)}' target='_blank' rel='noreferrer'>${esc(a.title)}</a><div class='meta'>${esc(a.source)} ${esc(a.published)}</div></div>`).join("");document.getElementById("modal").classList.remove("hidden")}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if parsed.path in ("/static/report-card-d.png", "/static/macro-card.png"):
