...
function cleanReportText(text){text=String(text||"").trim();text=text.replace(/^투자의견은\s*[^.。]+[.。]?\s*/,"").replace(/^투자의견\s*[^핵심\n]+\s*/,"");text=text.replace(/^핵심\s*[:：]\s*/,"").replace(/^핵심 근거\s*[:：]\s*/,"");return text.trim()}
function splitReportSummary(summary){summary=String(summary||"").replace(/\s*\/\s*/g,"\n").trim();const parts=summary.split(/\s*핵심\s*[:：]\s*/);const head=cleanReportText(parts.length>1?parts[0]:"");const core=cleanReportText(parts.length>1?parts.slice(1).join(" 핵심: "):summary);return {head,core}}
...
function renderReportDetail(d,report={}){const close=d.closeSeries||[];const targets=d.targetSeries||[];const flows=d.flowSeries||[];const kws=(report.keywords||[]).slice(0,8).map(k=>`<span class='chip'>${esc(k.keyword)}</span>`).join("");const summaryParts=splitReportSummary(report.summary||"");const reasonTexts=(report.reasons||[]).map(x=>x&&x.reason_text||"");const blocks=uniqueTextBlocks([summaryParts.head,summaryParts.core,report.target_price_reason,report.risk_summary,...reasonTexts]).slice(0,6);const opinion=report.investment_opinion?`<div class='chart-pill'><span>투자의견</span><b>${esc(report.investment_opinion)}</b></div>`:"";const target=report.target_price?`<div class='chart-pill'><span>목표가</span><b>${num(report.target_price)}원</b></div>`:"";const current=report.current_price_at_report_date?`<div class='chart-pill'><span>현재가</span><b>${num(report.current_price_at_report_date)}원</b></div>`:"";return `<div class='section-note'><b>${esc(report.stock_name||d.stockName||d.stockCode)}</b><br>${esc(report.title||"")}<br>${esc(report.securities_firm||"")} / ${esc(report.report_date||d.reportDate||"")}</div><div class='chart-summary'>${opinion}${target}${current}</div>${kws?`<div class='meta'>${kws}</div>`:""}${blocks.length?`<div class='section-note'><b>보고서 요약</b><ul>${blocks.map(x=>`<li>${esc(x)}</li>`).join("")}</ul></div>`:""}<div class='section-note'>종가 ${close.length}일 / 목표가 ${targets.length}건 / 수급 ${flows.length}일</div><div class='meta'>종가 vs 목표가 추이</div>${priceTargetSvg(close,targets)}<div class='meta'>외국인/기관 순매수 추이</div>${flowSvg(flows)}`}
