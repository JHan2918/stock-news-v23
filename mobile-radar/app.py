...
function renderReportDetail(d,report={}){const close=d.closeSeries||[];const targets=d.targetSeries||[];const flows=d.flowSeries||[];const reasons=(report.reasons||[]).filter(x=>x&&x.reason_text).slice(0,3).map(x=>`<li>${esc(x.reason_keyword||"")} ${esc(x.reason_text||"")}</li>`).join("");const kws=(report.keywords||[]).slice(0,8).map(k=>`<span class='chip'>${esc(k.keyword)}</span>`).join("");const summary=report.summary||"";const targetReason=report.target_price_reason||"";const risk=report.risk_summary||"";return `<div class='section-note'><b>${esc(report.stock_name||d.stockName||d.stockCode)}</b><br>${esc(report.title||"")}<br>${esc(report.securities_firm||"")} / ${esc(report.report_date||d.reportDate||"")} / ${esc(report.investment_opinion||"")}</div>${kws?`<div class='meta'>${kws}</div>`:""}${summary?`<div class='section-note'><b>요약</b><br>${esc(summary)}</div>`:""}${targetReason?`<div class='section-note'><b>목표가 이유</b><br>${esc(targetReason)}</div>`:""}${risk?`<div class='section-note'><b>리스크</b><br>${esc(risk)}</div>`:""}${reasons?`<div class='section-note'><b>추가 근거</b><ul>${reasons}</ul></div>`:""}<div class='section-note'>종가 ${close.length}일 / 목표가 ${targets.length}건 / 수급 ${flows.length}일</div><div class='meta'>종가 vs 목표가 추이</div>${priceTargetSvg(close,targets)}<div class='meta'>외국인/기관 순매수 추이</div>${flowSvg(flows)}`}
